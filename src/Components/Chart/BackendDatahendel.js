...
const PostData = (pathlocation, userIdentify, Data, setdataisLoaded, dataisLoaded, e) => {
    fetch(`https://graphi-server-production.up.railway.app/api/v1/grap/${pathlocation}/?user=${userIdentify}`, {
        method: 'POST',
        headers: {
...
const UpdateData = (index, pathlocation, value, setback) => {
    ...
    if (id) {
        fetch(`https://graphi-server-production.up.railway.app/api/v1/grap/${pathlocation}/${id}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
